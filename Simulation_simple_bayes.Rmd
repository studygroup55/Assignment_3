...
SimulateData <- function(n_agents, n_trials) {
  for (i in 1:n_agents) {
    ...
    for (j in 1:n_trials) {
      ...
      GroupRating <- FirstRating + Feedback #GroupRating is the rating of the self + the feedback
      while (GroupRating < 1 | GroupRating > 8) { #While GroupRating is outside of range of 1 and 8, redo Feedback and recalculate GroupRating
        Feedback <- round(runif(1, -3, 3), 0)
      ...
      SecondRating_transformed <- inv_logit_scaled(bias + 0.5*logit_scaled(FirstRating_transformed) + 0.5*logit_scaled(GroupRating_transformed)) #Simple bayes. Same weight so still a simple Bayes, but we want to take an average between the two sources of evidence. GroupRatingwise we will put too much weight on the evidence. NB bias is not transformed because we defined it on this scale already.

...
SimulateData <- function(n_agents, n_trials) {
  for (i in 1:n_agents) {
    ...
    for (j in 1:n_trials) {
      ...
      SecondRating_transformed <- inv_logit_scaled(bias + 0.7*logit_scaled(FirstRating_transformed) + 0.3*logit_scaled(GroupRating_transformed)) #Simple bayes. Same weight so still a simple Bayes, but we want to take an average between the two sources of evidence. Otherwise we will put too much weight on the evidence. NB bias is not transformed because we defined it on this scale already.
...
mod_simpleBayes <- cmdstan_model("SocialConf1_ass3.stan", cpp_options = list(stan_threads = TRUE),
